...
async def get_route_by_stations(body=Body()):
    ...
    try:
        ...
        result_transfers=[]
        for id,transfer in enumerate(grouped_transfers+directs):
            ...
            for sub_transfer in transfer:
                if not sub_transfer["is_transfer"]:
                    ...
                    transport.append(sub_transfer["route"]["transport"])
            ...
            result_transfers.append({
                ...
                "duration":duration,
                "price":price,
                "arrival":transfer[-1]["route"]["arrival"],
                "departure":transfer[0]["route"]["departure"],
                "transport":transport
            })

        print(len(filtered_transfers))
        print(sum(len(transfer) for transfer in transfers))
        return json.dumps({"data": result_transfers})
